...
const ChatList = ({ currentUserId }) => {
  ...
  return (
    <div className="chat-list">
      {chats.map(chat => (
        <Link to={`/chats/${chat.userId}`} key={chat.userId} className="chat-item">
...
const ChatsPage = () => {
  ...
  return (
    <div className="chats-page">
      <h1>{userId ? "Чат с пользователем" : "Мои чаты"}</h1>
      {currentUserId ? (
        userId ? (
          <Chats currentUserId={currentUserId} selectedUserId={userId} />
        ) : (
